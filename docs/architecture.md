...
- **Captured.** The fence remembers a *shadow* — where the person believes the pointer is on their physical screen — and the *target* where it last placed the cursor on the display. Inside the display, each event's offset from the target moves the shadow one to one. At or beyond a display edge, relative event deltas preserve movement that absolute coordinates may clip or relocate, including both axes of corner exits. The shadow is mapped back onto the display and the event is rewritten to that point. Leaving the stage rectangle, another window covering the stage, interaction being switched off, or pushing against a display edge releases the pointer at the shadow, clamped to a physical display. An edge push places it at least one point outside the picture without discarding overshoot.
- **Release handoff.** After a release, queued events can still carry virtual-display coordinates. Until an event reports physical coordinates, those events continue from the release point using relative movement. They must not go through ordinary free-pointer fencing, which would jump to the physical monitor edge nearest the virtual display.
...
Before delivering a captured primary mouse-down, `InputBridge` hit-tests its mapped virtual point using `NSWindow.windowNumber(at:belowWindowWithWindowNumber:)`. Both a zero result and a window at or below the desktop-icon layer identify a desktop surface: Finder can return a nonzero window ID for its desktop. Only the hit window's layer is queried, and only on primary down. `WallpaperClickGuard` consumes that down and its matching drag/up events, even if the pointer subsequently leaves the stage, preventing an orphaned gesture from reaching a different window or physical display. Normal Finder windows, desktop widgets, menus, and the Dock remain interactive. Control-click and secondary buttons retain desktop context menus. Ordinary clicks outside the stage are not hit-tested or suppressed. Consumed drags explicitly warp the cursor because there is no delivered event to update its position. This avoids invoking macOS's session-wide reveal-desktop action without changing a global preference.
